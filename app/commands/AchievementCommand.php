...
final class AchievementCommand extends BaseCommand
{
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$members = $this->database->query('SELECT user_id FROM akce_member JOIN akce ON akce_id = akce.id WHERE sequence_id IN (6,10,13,16,73,278) AND NOW() > date_end AND deleted_by IS NULL AND enable = 1 AND user_id NOT IN (SELECT user_id FROM achievement_users WHERE achievement_id = 15) GROUP BY user_id HAVING COUNT(user_id) >=3');
		foreach ($members as $member) {
